  ...
  return (
    <section ref={sectionRef} className="py-20 bg-white text-black overflow-x-hidden overflow-y-hidden relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 sm:grid-cols-2 mb-16">
          {problems.map((problem, index) => (
            <div
              ...
              className={`cursor-pointer border rounded-xl p-6 shadow-sm transition-all duration-300 ${activeTab === index ? 'border-[#1E3A8A] bg-blue-50' : 'border-gray-200 hover:bg-gray-50'}`}
            >
              <div className="flex gap-4 items-start">
                <div className="w-10 h-10 flex items-center justify-center bg-[#1E3A8A] text-white rounded-md">
                  {problem.icon}
                </div>
                <div>
                  <h4 className="text-lg font-semibold text-[#1E3A8A]">{problem.title}</h4>
                  <p className="text-sm text-gray-600">{problem.description}</p>
                  {activeTab === index && (
                    <>
                      <p className="text-sm text-gray-700 mt-2 animate-fade-in-scale">{problem.detail}</p>
                      <div className="mt-2 p-2 border border-red-300 bg-red-50 text-sm text-red-600 rounded animate-fade-in">
                        ⚠ {problem.impact}
                      </div>
                    </>
                  )}
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-3xl p-6 sm:p-8 lg:p-12 transition-all duration-1000 relative overflow-hidden">
          ...
          <div className="absolute bottom-10 left-10 w-24 h-24 sm:w-28 sm:h-28 opacity-5">
            <Cpu className="w-full h-full text-yellow-400 animate-spin" style={{ animationDuration: '20s' }} />
          </div>

          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-full mb-6 border border-[#1E3A8A]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {benefits.map((benefit, index) => (
              ...
              >
                <div className="relative bg-white border border-gray-200 rounded-2xl p-6 h-full hover:scale-105 hover:shadow-lg hover:border-[#1E3A8A] transition-all duration-300 overflow-hidden">
                  <div className="relative flex items-center justify-center w-12 h-12 bg-[#1E3A8A] rounded-xl mb-4 shadow-sm group-hover:bg-yellow-400 transition-colors duration-300">
                    <div className="text-white group-hover:text-[#1E3A8A] transition-colors duration-300">
                      {benefit.icon}
                    </div>
                  </div>
                  <div className="relative">
                    <h4 className="text-lg font-bold text-[#1E3A8A] mb-3 group-hover:text-yellow-400 transition-colors duration-300">
                      {benefit.title}
                    </h4>
          ...
          <div className="text-center">
            ...
            >
              <button className="bg-yellow-400 hover:bg-yellow-500 text-[#1E3A8A] font-bold px-6 sm:px-8 py-3 sm:py-4 rounded-full hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl text-sm sm:text-base">
