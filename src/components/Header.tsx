...
const Header = () => {
  ...
  // Dynamic background classes based on scroll state and device type
  const getHeaderBackground = () => {
    if (isScrolled) {
      if (isMobile) {
        return 'backdrop-blur-xl bg-blue-200/95 shadow-2xl border border-gray-200/50';
      } else {
        return 'backdrop-blur-xl bg-white/95 shadow-2xl border border-gray-200/50';
      }
    }
    return '';
  };
  ...
  const getTextColor = () => {
    if (isScrolled) {
      return 'text-gray-800 hover:text-yellow-400';
    }
    return 'text-white hover:text-yellow-400 drop-shadow-md';
  };
      ...
      <header className="fixed top-0 left-0 right-0 z-30 w-full h-fit">
        <div
          className={`transition-all duration-700 ease-out mx-auto transform z-30
            ${isScrolled
              ? `w-full max-w-[95%] sm:max-w-[92%] lg:max-w-6xl mt-2 px-3 sm:px-4 lg:px-6 rounded-xl sm:rounded-2xl ${getHeaderBackground()}`
              ...
              <div className="flex items-center space-x-4 xl:space-x-6">
                {navigationItems.map((item, index) => (
                  <button
                    ...
                    className={`nav-item transition-all duration-500 font-poppins font-medium hover:scale-105 relative group text-sm xl:text-base whitespace-nowrap transform hover:-translate-y-1 ${getTextColor()}`}
            ...
            <div className="flex flex-col pt-24 pb-6 px-4 space-y-4 overflow-y-auto">
              <nav className="flex-1 flex flex-col justify-center space-y-1">
                {navigationItems.map((item, index) => (
                  <button
                    key={item.id}
                    onClick={() => scrollToSection(item.id)}
                    className="text-gray-800 hover:text-yellow-500 transition-all duration-500 font-poppins font-medium py-4 px-6 text-left hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent rounded-2xl transform hover:translate-x-2 hover:scale-105 w-full text-xl relative overflow-hidden group"
                    style={{
                      animation: `fadeInUp 0.5s ease-out ${index * 100}ms both`
                    }}
                  >
                    <span className="relative z-10">{item.label}</span>
                    <span className="absolute inset-0 bg-gradient-to-r from-blue-100/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl"></span>
                  </button>
                ))}
              </nav>
              ...
              <div className="pt-6">
                <Button
                  ...
                  className="bg-yellow-400 text-black font-poppins font-semibold px-6 py-3 rounded-full w-full text-base sm:text-lg hover:shadow-xl transition-all duration-500 transform hover:scale-105 hover:-translate-y-1 relative overflow-hidden"
                  style={{
